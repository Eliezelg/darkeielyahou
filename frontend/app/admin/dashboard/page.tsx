...
export default function AdminDashboard() {
  ...
  const handleExport = async () => {
    try {
      const response = await callBackendApi('/api/admin/requests/export');
      ...
      toast({
        title: 'Erreur',
        description: 'Impossible d\'exporter les demandes'
      });
    }
  };
  ...
  return (
    <div className="container mx-auto">
      <div className="flex justify-between items-center mb-6">
        ...
        <Button onClick={handleExport} disabled={isLoading || requests.length === 0}>
          Exporter en Excel
        </Button>
      </div>
      ...
      <Tabs defaultValue="all" className="w-full" onValueChange={setActiveTab}>
        ...
        <TabsContent value={activeTab}>
          <Card>
            <CardHeader>
              <CardTitle>Demandes de formulaire</CardTitle>
            </CardHeader>
            <CardContent>
              {isLoading ? (
